/** OpenKnowledge preview iframe tokens (packages/core/src/constants/preview-theme-tokens.ts) */
...
function tokensToCss(tokens, theme) {
  return tokens.map((t) => `${t.name}:${t[theme]};`).join('');
}

export function siteTokensCss(theme = 'light') {
  return `:root{${tokensToCss(PREVIEW_THEME_TOKENS, theme)}}`;
}

export function previewIframeTokensCss() {
  const light = tokensToCss(PREVIEW_THEME_TOKENS, 'light');
  return `:root{${light}}body{margin:0;color:var(--foreground);background:var(--background);font-family:system-ui,sans-serif}`;
}
